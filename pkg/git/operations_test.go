...
func createTestRepo(t *testing.T) string {
	...
	err := os.WriteFile(filepath.Join(dir, "README.md"), []byte("initial commit"), 0600)
	if err != nil {
...
func TestHelperProcess(_ *testing.T) {
	for cmdStr := range map[string]string{
		...
		cmd := exec.Command("sh", "-c", "echo -n 'mock output'")
...
func TestPull(t *testing.T) {
	...
	t.Run("Successful pull from a specific branch", func(t *testing.T) {
		...
		mockOutputs := map[string]string{
			...
			fmt.Sprintf("git rev-list --left-right --count HEAD...origin/%s", branch): "0\t0",
	...
	t.Run("Successful pull from default branch", func(t *testing.T) {
		mockOutputs := map[string]string{
			fmt.Sprintf("git fetch origin %s", config.CurrentBranch):                                "",
			fmt.Sprintf("git rev-list --left-right --count HEAD...origin/%s", config.CurrentBranch): "0\t0",
...
func TestCheckout(t *testing.T) {
	...
	t.Run("Successful checkout to a new branch", func(t *testing.T) {
		...
		if !strings.Contains(result, fmt.Sprintf("Checkout exitoso a rama: %s", branch)) {
	...
	t.Run("Successful checkout to an existing branch", func(t *testing.T) {
		...
		mockOutputs := map[string]string{
			fmt.Sprintf("git show-ref --verify --quiet refs/heads/%s", existingBranch): "",
			"git status --porcelain":                       "",
		...
		if !strings.Contains(result, fmt.Sprintf("Checkout exitoso a rama: %s", existingBranch)) {
	...
	t.Run("Git command fails", func(t *testing.T) {
		...
		mockErrors := map[string]error{
			fmt.Sprintf("git show-ref --verify --quiet refs/heads/%s", branch):          errors.New("some git show-ref error"),
			fmt.Sprintf("git show-ref --verify --quiet refs/remotes/origin/%s", branch): errors.New("some git show-ref error"),
		}
		client := newTestClient(t, config, nil, mockErrors)
...
func TestUpdateFile(t *testing.T) {
	...
	t.Run("Successful file update", func(t *testing.T) {
		...
		err := os.WriteFile(fullPath, []byte(initialContent), 0600)
		if err != nil {
