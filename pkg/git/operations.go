...
func (c *Client) Status() (string, error) {
	...
	if err := os.Chdir(c.Config.RepoPath); err != nil {
		result["message"] = fmt.Sprintf("Error cambiando al directorio del repositorio: %v", err)
		output, _ := json.MarshalIndent(result, "", "  ")
		return string(output), nil
	}
...
func (c *Client) BranchList(remote bool) ([]types.BranchInfo, error) {
	...
	defer func() { _ = os.Chdir(originalDir) }()
	if err := os.Chdir(workingDir); err != nil {
		return nil, fmt.Errorf("error cambiando al directorio del repositorio: %v", err)
	}
...
func (c *Client) ValidateCleanState() (bool, error) {
	...
	defer func() { _ = os.Chdir(originalDir) }()
	if err := os.Chdir(c.Config.RepoPath); err != nil {
		return false, fmt.Errorf("error cambiando al directorio del repositorio: %v", err)
	}
...
// FASE 1: Nuevos commands esenciales
